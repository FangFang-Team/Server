...
npy_file_path = "voc12/cls2_labels.npy"
# npy_file_path = os.path.abspath(os.path.join(os.getcwd(), "../voc12/cls_labels.npy"))
# cls_labels_dict = np.load('voc12/cls_labels.npy', allow_pickle=True).item()
cls_labels_dict = np.load(npy_file_path, allow_pickle=True).item()
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.type == "train_set":
            img_path = os.path.join(self.voc12_root, f"train_images/{name_str}")
        else:
            img_path = os.path.join(self.voc12_root, f"val_images/{name_str}")
